...
import { ArmedState, ActionableState, DisplayState, ON, OFF, LOCKED, UNLOCKED } from '@/lib/mappings/definitions';
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        actionIntent: {
          type: "string",
          enum: ["turn_on", "turn_off", "lock", "unlock", "list_all"],
          description: "Specific action intent: 'turn_on' for turn on requests, 'turn_off' for turn off requests, 'lock' for lock requests, 'unlock' for unlock requests, 'list_all' to show all available actions"
        }
      }
    }
  },
...
export async function executeFunction(
  ...
  switch (name) {
    ...
    case 'lock_device':
      return lockDevice(orgDb, args);
      
    case 'unlock_device':
      return unlockDevice(orgDb, args);
  ...
  try {
    filteredDevices.forEach((device: any, index: number) => {
      ...
      console.log(`[findControllableDevices] Found action handler for category: ${device.connector.category}`);
        ...
        // Check if device supports lock/unlock actions
        ...
        console.log(`[findControllableDevices] Device "${device.name}" controllability: canTurnOn=${canTurnOn}, canTurnOff=${canTurnOff}, canLock=${canLock}, canUnlock=${canUnlock}`);

        
        if (canTurnOn || canTurnOff || canLock || canUnlock) {
        ...
        // Add actions based on intent (original state-aware logic)
          ...
          // Add lock/unlock actions based on capability and current state
          const shouldShowLock = (actionIntent === "list_all" || actionIntent === "lock") && 
                                canLock && currentState !== LOCKED;
          const shouldShowUnlock = (actionIntent === "list_all" || actionIntent === "unlock") && 
                                  canUnlock && currentState !== UNLOCKED;
          
          if (shouldShowLock) {
            actions.push({
              id: `device-${device.id}-lock`,
              type: 'device',
              label: `Lock ${device.name}`,
              icon: 'Lock',
              metadata: {
                internalDeviceId: device.id,
                deviceName: device.name,
          ...
          if (shouldShowUnlock) {
            actions.push({
              id: `device-${device.id}-unlock`,
              type: 'device',
              label: `Unlock ${device.name}`,
              icon: 'Unlock',
              metadata: {
                internalDeviceId: device.id,
                deviceName: device.name,
        ...
        console.log(`[findControllableDevices] Device "${device.name}" is not controllable (no supported actions)`);
      }
    });
  } catch (error) {
