...
} from "@/lib/auth/withOrganizationAuth";
...
function formatDevice(device: DeviceWithConnectorInfo): DeviceResponse {
  return {
    ...
    name: device.name,
    type: device.type,
    lastStateUpdate: device.lastStateUpdate
      ? device.lastStateUpdate.toISOString()
      : null,
...
export const GET = withOrganizationAuth(
  async (request, authContext: OrganizationAuthContext) => {
    try {
      ...
      if (requestedDeviceId) {
        ...
        if (deviceRow.serverId) {
          const pikoServerResult = await db
            .select()
            .from(pikoServers)
        ...
        const deviceTypeInfo = getDeviceTypeInfo(
          deviceRow.connector?.category || "unknown",
          deviceRow.type
        );
        ...
        const singleDevice: DeviceWithConnector = {
          ...
          locationName: (deviceRow as any).locationName ?? undefined,
        ...
        const pikoServersResult =
          serverIds.length > 0
            ? await db
                .select()
                .from(pikoServers)
                .where(inArray(pikoServers.serverId, serverIds))
            : [];

        const pikoServersMap = new Map(
          pikoServersResult.map((server) => [
            server.serverId,
            server as PikoServer,
          ])
        );

        // Map results to DeviceWithConnector format
        const devicesWithDetails: DeviceWithConnector[] = devicesResult.map(
          (deviceRow: any) => {
            const pikoServerDetails = deviceRow.serverId
              ? pikoServersMap.get(deviceRow.serverId)
              : undefined;
      ...
      for (const connector of allConnectors) {
        try {
          ...
          try {
            ...
            console.error(
              `[API Sync] Error parsing config for connector ${connector.name} (ID: ${connector.id}):`,
              e
            );
            errors.push({
          ...
          switch (connector.category) {
            ...
            default:
              console.warn(
                `[API Sync] Skipping unsupported connector category: ${connector.category}`
              );
              continue;
          }
          ...
          syncedCount += result;
          console.log(
            `[API Sync] Synced ${result} devices from ${connector.category} connector ${connector.name}`
          );
          ...
          const errorMessage =
            connectorError instanceof Error
              ...
              : "Unknown error";
          console.error(
            `[API Sync] Error syncing connector ${connector.name}:`,
            errorMessage
          );
          errors.push({
            connectorName: connector.name,
...
async function fetchDevicesForOrganization(
  orgDb: any
): Promise<DeviceWithConnector[]> {
  const devicesResult = await orgDb.devices.findAll();
  ...
  const pikoServersMap = new Map(
    pikoServersResult.map((server) => [server.serverId, server as PikoServer])
  );
...
async function syncYoLinkDevices(
  ...
  try {
    ...
    const yolinkDevicesFromApi = await yolinkDriver.getDeviceList(
      connectorId,
      config
    );
    console.log(
      `Found ${yolinkDevicesFromApi.length} YoLink devices from API for metadata sync.`
    );
    ...
    if (staleDeviceIds.length > 0) {
      console.log(
        `[API Sync YoLink] Deleting ${staleDeviceIds.length} stale devices: ${staleDeviceIds.join(", ")}`
      );
    ...
    for (const device of yolinkDevicesFromApi) {
      ...
      if (typeof device.state === "object" && device.state !== null) {
        initialStatusFromList =
          (device.state as any).state ?? (device.state as any).power ?? null;
      }
      ...
      if (canFetchState) {
        try {
          console.log(
            `  [API Sync YoLink] Fetching fresh state for ${device.type} ${device.deviceId}...`
          );
          ...
          if (stateData && typeof stateData === "object") {
            ...
            if (
              typeof stateData.online === "boolean" &&
              stateData.online === false
            ) {
              calculatedDisplayState = OFFLINE;
              console.log(
                `[API Sync YoLink] Device ${device.deviceId} (${device.type}) is offline.`
              );
            } else {
              // For online devices, extract state from the state data
              const rawStateString = getRawStateStringFromYoLinkData(
                stdTypeInfo,
                stateData
              );
              ...
              if (rawStateString) {
                ...
                if (rawStateString === "error") {
                  ...
                  stdTypeInfo.type === DeviceType.Switch ||
                  stdTypeInfo.type === DeviceType.Outlet
                ) {
                  if (rawStateString === "open" || rawStateString === "on") {
                    intermediateState = BinaryState.On;
                  } else if (
                    rawStateString === "closed" ||
                    rawStateString === "off"
                  ) {
                    intermediateState = BinaryState.Off;
                  }
                } else if (
                  stdTypeInfo.type === DeviceType.Sensor ||
                  stdTypeInfo.type === DeviceType.WaterValveController
                ) {
                ...
                if (intermediateState !== undefined) {
                  ...
                  const simpleMap = CANONICAL_STATE_MAP.simple as Record<
                    string,
                    DisplayState
                  >;
                  calculatedDisplayState = simpleMap[intermediateState];
                  console.log(
                    `[API Sync YoLink] Device ${device.deviceId} (${device.type}) state: ${rawStateString} -> ${intermediateState} -> ${calculatedDisplayState}`
                  );
                } else {
                  console.warn(
                    `[API Sync YoLink] Could not map raw state "${rawStateString}" to intermediate state for ${device.deviceId} (${device.type})`
                  );
                }
              } else if (
                typeof stateData?.online === "boolean" &&
                stateData.online === true
              ) {
                calculatedDisplayState = ONLINE;
                console.log(
                  `[API Sync YoLink] Device ${device.deviceId} (${device.type}) is online, no further distinct physical state found. Marked as ONLINE.`
                );
              } else if (!calculatedDisplayState) {
                console.warn(
                  `[API Sync YoLink] Could not determine state for ${device.deviceId} (${device.type}) from response:`,
                  stateData
                );
              }
            }
          ...
          const errorMessage =
            stateError instanceof Error
              ...
              : String(stateError);
          console.error(
            `  [Error] Failed to fetch state for ${device.type} ${device.deviceId}:`,
            errorMessage
          );
          console.warn(
            `[API Sync YoLink] State for ${device.deviceId} (${device.type}) will not be updated due to API error during state fetch.`
          );
        }
      }
    ...
    console.log(
      `[API Sync YoLink] Completed sync for connector ${connectorId}: processed ${processedCount}, deleted ${deletedCount}`
    );
    return processedCount;
  } catch (error) {
    console.error(
      `[API Sync YoLink] Error syncing YoLink devices for connector ${connectorId}:`,
      error
    );
    throw error;
  }
...
async function syncPikoDevices(
  ...
  try {
    ...
    console.log(
      `Found ${cameras.length} devices and ${servers.length} servers from Piko API`
    );
    ...
    if (apiDeviceIds.length > 0) {
      await db.delete(devices).where(
        and(
          eq(devices.connectorId, connectorId),
          sql`${devices.deviceId} NOT IN (${sql.join(
            apiDeviceIds.map((id) => sql`${id}`),
            sql`, `
          )})`
        )
      );
      console.log(
        `[API Sync Piko] Deleted stale devices not in current API response.`
      );
    } else {
    ...
    console.log(
      `[API Sync Piko] Completed sync for connector ${connectorId}: processed ${processedCount} cameras`
    );
    return processedCount;
  } catch (error) {
    console.error(
      `[API Sync Piko] Error syncing Piko devices for connector ${connectorId}:`,
      error
    );
    throw error;
  }
...
async function syncGeneaDevices(
  ...
  try {
    ...
    if (apiDeviceIds.length > 0) {
      ...
      console.log(
        `[API Sync Genea] Deleted stale devices not in current API response.`
      );
    } else {
      // If no devices from API, delete all devices for this connector
      await db.delete(devices).where(eq(devices.connectorId, connectorId));
      console.log(
        `[API Sync Genea] Deleted all devices (no devices from API).`
      );
    }
    ...
    console.log(
      `[API Sync Genea] Completed sync for connector ${connectorId}: processed ${processedCount} devices`
    );
    return processedCount;
  } catch (error) {
    console.error(
      `[API Sync Genea] Error syncing Genea devices for connector ${connectorId}:`,
      error
    );
    throw error;
  }
