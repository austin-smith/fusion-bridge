...
import { AutomationConfigSchema, type AutomationConfig, type AutomationAction } from '@/lib/automation-schemas';
...
// Define AutomationFormData locally within this component
interface AutomationFormData {
    id: string;
    name: string;
    enabled: boolean;
    sourceConnectorId: string | null; // Allow null for new forms
    configJson: AutomationConfig;
    createdAt: Date; // Keep these for potential display?
    updatedAt: Date;
}
...
const FormSchema = z.object({
    ...
    enabled: z.boolean(),
    sourceConnectorId: z.string().uuid("Source connector must be selected"),
...
export default function AutomationForm({
    ...
    const initialActions = initialData.configJson?.actions?.map((action: AutomationAction) => {
        const params = action.params as any;
        if ((action.type === 'createEvent' || action.type === 'createBookmark') && params) {
            const targetConnectorIdValue = params.targetConnectorId ?? params.targetConnectorId ?? '';
    ...
    const form = useForm<AutomationFormValues>({
        ...
        defaultValues: {
            ...
            sourceConnectorId: initialData.sourceConnectorId || '',
            ...
            } : { sourceEntityTypes: [], eventTypeFilter: '', actions: [] }
        },
        mode: 'onTouched',
    });
    ...
    const handleInsertToken = (
        ...
        if ((fieldName as string).startsWith('headers.') && headerIndex !== undefined) {
             ...
             } else { console.error("Invalid header field name:", fieldName); return; }
        } else { currentFieldName = `config.actions.${actionIndex}.params.${fieldName as Exclude<ActionParamFieldNames, `headers.${number}.keyTemplate` | `headers.${number}.valueTemplate`>}`; }
        try {
    ...
    const onSubmit = async (data: AutomationFormValues) => {
        ...
        const processedData = { ...data, config: { ...data.config, actions: data.config.actions.map((action: AutomationAction) => {
        ...
        try {
            const response = await fetch(apiUrl, { method: httpMethod, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(processedData) });
            if (!response.ok) {
    ...
    const sourceConnectors = availableConnectors.filter(c => c.category === 'yolink');
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <FormField /* Name */
                                    ...
                                    <FormItem>
                                        ...
                                        <FormControl><Input {...field} disabled={isLoading} className={cn(fieldState.invalid && 'border-destructive')} /></FormControl>
                                    </FormItem>
                                )} />
                                        ...
                                        <Select onValueChange={field.onChange} defaultValue={field.value} disabled={isLoading}>
                                            ...
                                            <SelectContent>
                                                {sourceConnectors.map(connector => (
                                                    <SelectItem key={connector.id} value={connector.id}>{connector.name} ({connector.category})</SelectItem>
                                                ))}
                                            </SelectContent>
                                        </Select>
                                    </FormItem>
                         ...
                         <CardContent className="space-y-4">
                                ...
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <MultiSelectCombobox
                                            ...
                                            className={cn(fieldState.invalid && 'border border-destructive rounded-md')}
                                        />
                                    </FormControl>
                                    <FormDescription className={descriptionStyles}>Select which types of devices can trigger this automation.</FormDescription>
                                </FormItem>
                            )} />
                            <FormField /* Event Type Filter */
                                ...
                                <FormItem>
                                    ...
                                    <FormControl><Input {...field} placeholder="e.g., DoorSensor.Report, LeakSensor.*" disabled={isLoading} className={cn(fieldState.invalid && 'border-destructive')} /></FormControl>
                                    <FormDescription className={descriptionStyles}>Use * as wildcard. Leave blank for all.</FormDescription>
                                </FormItem>
                            )} />
                        </CardContent>
                    </Card>
                    ...
                    <div>
                                    ...
                                    <Card key={fieldItem.id} className="relative border border-dashed pt-8">
                                        ...
                                        <CardContent className="space-y-4">
                                                    ...
                                                    <FormItem>
                                                        ...
                                                        <Select
                                                            onValueChange={(value) => {
                                                                ...
                                                                if (newType === 'createEvent') { newAction = { type: 'createEvent', params: { sourceTemplate: '', captionTemplate: '', descriptionTemplate: '', targetConnectorId: '' } };
                                                                } else if (newType === 'createBookmark') { newAction = { type: 'createBookmark', params: { nameTemplate: '', descriptionTemplate: '', durationMsTemplate: '5000', tagsTemplate: '', targetConnectorId: '' } };
                                                                } else if (newType === 'sendHttpRequest') { newAction = { type: 'sendHttpRequest', params: { urlTemplate: '', method: 'GET', headers: [], contentType: 'text/plain', bodyTemplate: '' } };
                                                                } else { console.warn(`Unexpected action type: ${value}.`); newAction = { type: 'createEvent', params: { sourceTemplate: '', captionTemplate: '', descriptionTemplate: '', targetConnectorId: '' } }; }
                                                                form.setValue(`config.actions.${index}`, newAction, { shouldValidate: false, shouldDirty: true, shouldTouch: false });
                                                                field.onChange(newType);
                                                            }}
                                                            defaultValue={field.value}
                                                        >
                                                            <FormControl><SelectTrigger className={cn(fieldState.invalid && 'border-destructive')}><SelectValue placeholder="Select Action Type" /></SelectTrigger></FormControl>
                                                            <SelectContent>
                                            ...
                                            {(actionType === 'createEvent' || actionType === 'createBookmark') && (
                                                 <FormField /* Target Connector (Piko) */
                                                     control={form.control}
                                                     name={paramFieldName('targetConnectorId')}
                                                     render={({ field, fieldState }) => (
                                                     <FormItem>
                                            ...
                                            <div className="space-y-2 border-l-2 pl-4 ml-1 border-muted">
                                                ...
                                                {actionType === 'sendHttpRequest' && (
                                                    <SendHttpRequestActionFields actionIndex={index} handleInsertToken={handleInsertToken} />
                                                )}
                        ...
                        <Button type="button" variant="outline" size="sm" className="mt-4" onClick={() => appendAction({ type: 'createEvent', params: { sourceTemplate: '', captionTemplate: '', descriptionTemplate: '', targetConnectorId: '' } })} disabled={isLoading}>
