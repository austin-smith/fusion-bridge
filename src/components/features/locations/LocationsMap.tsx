    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const buildingIcon = await getLucideIconImage('Building2', {
                size: Math.round(size * 0.6), // Icon is 60% of total size
                color: iconColor,
                strokeWidth: 2
            });

            // Create canvas to composite the icon with backdrop
            ...
            // Draw circular backdrop with border
            ctx.beginPath();
            ctx.arc(center, center, radius - 2, 0, 2 * Math.PI);
            ctx.fillStyle = backdropColor;
            ctx.fill();

            // Reset shadow for border to avoid double shadow
            ctx.shadowColor = 'transparent';
            ctx.strokeStyle = borderColor;
            ctx.lineWidth = 2;
            ctx.stroke();

            // Draw the building icon centered
...
export default function LocationsMap(props: LocationsMapProps) {
    ...
    const { featureCollection, selectedFeatureCoordinates, viewState, dataBounds } = useMemo(() => {
        const featureCollection = locationsToFeatureCollection(locations) as any;
    ...
    return (
        <div className="w-full h-full rounded-lg overflow-hidden relative">
            ...
            <Map
                ref={mapRef}
                mapStyle={useMemo(() => getDefaultMapStyle(), []) as any}
                ...
                >
                    ...
                    {selectedLocationId && selectedFeatureCoordinates && (
                        <Layer
                            ...
                            type="symbol"
                            filter={["all", ["!has", "point_count"], ["==", "id", selectedLocationId]] as any}
